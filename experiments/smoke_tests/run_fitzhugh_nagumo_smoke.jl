## Load smoke configuration for FitzHugh-Nagumo system
...
include(joinpath(PROJECT_ROOT, "src", "dynamics", "fitzhugh_nagumo.jl"))
...
include(joinpath(PROJECT_ROOT, "src", "diagnostics", "fitzhugh_nagumo_diagnostics.jl"))
include(joinpath(PROJECT_ROOT, "src", "generators", "fitzhugh_nagumo_dataset_generator.jl"))

load_config(parts...) = JSON.parsefile(joinpath(PROJECT_ROOT, "configs", parts...))

function load_fitzhugh_nagumo_smoke_configs()
    return Dict(
        "benchmark" => load_config("benchmarks", "v1_core", "fitzhugh_nagumo_smoke_benchmark.json"),
        "system" => load_config("systems", "v1_core", "fitzhugh_nagumo_smoke.json"),
        "observation" => load_config("observations", "fitzhugh_nagumo_full_state_clean.json"),
        "split" => load_config("splits", "v1_core", "fitzhugh_nagumo_smoke_split_i.json"),
        "window" => load_config("windows", "v1_core", "fitzhugh_nagumo_smoke_windows.json"),
        "tasks" => load_config("tasks", "v1_core", "fitzhugh_nagumo_smoke_tasks.json")["tasks"],
    )
end

## Generate smoke raw trajectories and derived dataset objects

function run_fitzhugh_nagumo_smoke()
    configs = load_fitzhugh_nagumo_smoke_configs()
    spec, raw_trajectories = generate_fitzhugh_nagumo_raw_trajectories(configs["system"])
    observation_spec, observed_trajectories = generate_fitzhugh_nagumo_observed_trajectories(
        raw_trajectories,
        configs["observation"],
        spec.state_dim,
    )
    split = build_fitzhugh_nagumo_split(raw_trajectories, configs["split"])
    window_summary = build_fitzhugh_nagumo_window_summary(split, spec.trajectory_length, configs["window"])

    diagnostics = summarize_fitzhugh_nagumo_dataset(spec, raw_trajectories, observed_trajectories)
    enrich_fitzhugh_nagumo_diagnostics!(diagnostics, split, window_summary)

    output_paths = save_fitzhugh_nagumo_outputs(
...
function print_fitzhugh_nagumo_smoke_summary(result::AbstractDict)
    ...
    @printf("a / b / epsilon / I: %.6g / %.6g / %.6g / %.6g\n", spec.a, spec.b, spec.epsilon, spec.I)
    @printf("dt: %.6g\n", spec.dt)
    ...
    @printf("state range w: [%.6g, %.6g]\n", diagnostics["w_min"], diagnostics["w_max"])
    ...
    @printf("equilibrium count: %d\n", diagnostics["equilibrium_count"])
    @printf("equilibrium residual max: %.6e\n", diagnostics["equilibrium_residual_max"])
    @printf("threshold crossing counts: %s\n", string(diagnostics["threshold_crossing_counts"]))
    @printf("excursion trajectory count: %d\n", diagnostics["excursion_trajectory_count"])
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = run_fitzhugh_nagumo_smoke()
    print_fitzhugh_nagumo_smoke_summary(result)
    result["diagnostics"]["smoke_passed"] || error("FitzHugh-Nagumo smoke generation failed diagnostics")
end
