...
include(joinpath(PROJECT_ROOT, "src", "dynamics", "lorenz63.jl"))
...
include(joinpath(PROJECT_ROOT, "src", "diagnostics", "lorenz63_diagnostics.jl"))
include(joinpath(PROJECT_ROOT, "src", "generators", "lorenz63_dataset_generator.jl"))

load_config(parts...) = JSON.parsefile(joinpath(PROJECT_ROOT, "configs", parts...))

function load_lorenz63_smoke_configs()
    return Dict(
        "benchmark" => load_config("benchmarks", "v1_core", "lorenz63_smoke_benchmark.json"),
        "system" => load_config("systems", "v1_core", "lorenz63_smoke.json"),
        "observation" => load_config("observations", "lorenz63_full_state_clean.json"),
        "split" => load_config("splits", "v1_core", "lorenz63_smoke_split_i.json"),
        "window" => load_config("windows", "v1_core", "lorenz63_smoke_windows.json"),
        "tasks" => load_config("tasks", "v1_core", "lorenz63_smoke_tasks.json")["tasks"],
    )
end

## Run Lorenz '63 smoke generation pipeline

function run_lorenz63_smoke()
    configs = load_lorenz63_smoke_configs()
    spec, raw_trajectories = generate_lorenz63_raw_trajectories(configs["system"])
    observation_spec, observed_trajectories = generate_lorenz63_observed_trajectories(
        raw_trajectories,
        configs["observation"],
        spec.state_dim,
    )
    split = build_lorenz63_split(raw_trajectories, configs["split"])
    window_summary = build_lorenz63_window_summary(split, spec.trajectory_length, configs["window"])

    diagnostics = summarize_lorenz63_dataset(spec, raw_trajectories, observed_trajectories)
    enrich_lorenz63_diagnostics!(diagnostics, split, window_summary)

    output_paths = save_lorenz63_outputs(
...
function print_lorenz63_smoke_summary(result::AbstractDict)
    ...
    @printf("sigma / rho / beta: %.6g / %.6g / %.6g\n", spec.sigma, spec.rho, spec.beta)
    @printf("dt: %.6g\n", spec.dt)
    ...
    @printf("divergence: %.6g\n", diagnostics["divergence"])
    @printf("equilibrium count: %d\n", diagnostics["equilibrium_count"])
    @printf("double-wing trajectory count: %d\n", diagnostics["double_wing_trajectory_count"])
    @printf("wing switch counts: %s\n", string(diagnostics["wing_switch_counts"]))
    @printf("separation growth to t=3: %.6g\n", diagnostics["separation_growth_t3"])
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = run_lorenz63_smoke()
    print_lorenz63_smoke_summary(result)
    result["diagnostics"]["smoke_passed"] || error("Lorenz63 smoke generation failed diagnostics")
end
