...
function load_fitzhugh_nagumo_formal_configs()
    return Dict(
        "benchmark" => load_config("benchmarks", "v1_core", "fitzhugh_nagumo_formal_benchmark.json"),
        "system" => load_config("systems", "v1_core", "fitzhugh_nagumo_formal.json"),
        "observation" => load_config("observations", "fitzhugh_nagumo_full_state_clean.json"),
        "split" => load_config("splits", "v1_core", "fitzhugh_nagumo_formal_split_i.json"),
        "window" => load_config("windows", "v1_core", "fitzhugh_nagumo_formal_windows.json"),
        "tasks" => load_config("tasks", "v1_core", "fitzhugh_nagumo_formal_tasks.json")["tasks"],
    )
end

## Formal dataset generation entry point

function run_fitzhugh_nagumo_formal_generation()
    configs = load_fitzhugh_nagumo_formal_configs()
...
function print_fitzhugh_nagumo_formal_summary(result::AbstractDict)
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = run_fitzhugh_nagumo_formal_generation()
    print_fitzhugh_nagumo_formal_summary(result)
    result["diagnostics"]["formal_passed"] || error("FitzHugh-Nagumo formal generation failed diagnostics")
end
