...
## 2. Configuration and plotting helpers

load_config(parts...) = JSON.parsefile(joinpath(PROJECT_ROOT, "configs", parts...))

function save_linear_diagonal_plots(times, X, plot_dir)
    if !PLOTS_AVAILABLE
        @warn "Skipping plots because Plots.jl could not be loaded" exception = PLOTS_LOAD_ERROR[]
        return String[]
    end
    ...
    try
        ...
        timeseries_path = joinpath(plot_dir, "coordinate_timeseries.png")
        Plots.savefig(p1, timeseries_path)

        p2 = Plots.plot(times, log.(abs.(X') .+ eps()); xlabel = "t", ylabel = "log(|x_i(t)|)", title = "Log amplitudes")
        log_path = joinpath(plot_dir, "log_amplitudes.png")
        Plots.savefig(p2, log_path)

        return [timeseries_path, log_path]
    catch err
        @warn "Skipping plots because plot generation failed" exception = err
        return String[]
    end
end

## 3. Formal linear diagonal dataset generator

function generate_linear_diagonal_dataset(;
    dataset_version::AbstractString = "0.1.0-dev",
    system_config_name::AbstractString = "linear_diagonal_small.json",
    observation_config_name::AbstractString = "full_state_identity.json",
    split_config_name::AbstractString = "split_I_70_15_15_seed1.json",
    one_step_window_config_name::AbstractString = "one_step_lag1.json",
    rollout_window_config_name::AbstractString = "rollout_horizon20.json",
    one_step_task_config_name::AbstractString = "one_step_forecast.json",
    rollout_task_config_name::AbstractString = "multi_step_rollout.json",
    benchmark_config_name::AbstractString = "linear_diagonal_unit_internal.json",
    ...
    plot_files = save_linear_diagonal_plots(first(raw_trajectories).times, first(raw_trajectories).state_matrix, plot_dir)
    ...
    manifest_path = write_json_file(joinpath(manifest_dir, manifest_filename), manifest)
...
## 4. Direct script entry point

function print_generation_summary(result::AbstractDict)
    system_spec = result["system_spec"]
    ...
    @printf("plot files: %s\n", string(result["plot_files"]))
end

if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = generate_linear_diagonal_dataset()
