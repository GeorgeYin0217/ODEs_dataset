...
function load_lorenz63_standard_configs()
    return Dict(
        "benchmark" => load_config("benchmarks", "v1_core", "lorenz63_standard_benchmark.json"),
        "system" => load_config("systems", "v1_core", "lorenz63_standard.json"),
        "observation" => load_config("observations", "lorenz63_full_state_clean.json"),
        "split" => load_config("splits", "v1_core", "lorenz63_standard_split_i.json"),
        "window" => load_config("windows", "v1_core", "lorenz63_standard_windows.json"),
        "tasks" => load_config("tasks", "v1_core", "lorenz63_standard_tasks.json")["tasks"],
    )
end

## Standard dataset generation entry point

function run_lorenz63_standard_generation()
    configs = load_lorenz63_standard_configs()
...
function print_lorenz63_standard_summary(result::AbstractDict)
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    result = run_lorenz63_standard_generation()
    print_lorenz63_standard_summary(result)
    result["diagnostics"]["standard_passed"] || error("Lorenz63 standard generation failed diagnostics")
end
