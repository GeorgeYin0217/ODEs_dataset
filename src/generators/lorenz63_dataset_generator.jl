...
function lorenz63_project_path(project_root::AbstractString, relative_path::AbstractString)
    return joinpath(project_root, split(relative_path, '/')...)
end

## Manual smoke initial-condition set

function validate_lorenz63_manual_initial_conditions(system_config::AbstractDict)
    ...
    if domain_type == "manual_smoke_set"
        ...
        for x0 in values
            length(x0) == 3 || throw(ArgumentError("each Lorenz63 initial condition must have three entries"))
            x0_float = Float64.(x0)
    ...
    elseif domain_type == "manual_grid"
        ...
        throw(ArgumentError("unsupported Lorenz63 initial_condition_domain type: $(domain_type)"))
    end
    return true
end

function lorenz63_manual_initial_conditions(system_config::AbstractDict)
    validate_lorenz63_manual_initial_conditions(system_config)
    domain = system_config["initial_condition_domain"]
    domain_type = String(domain["type"])
...
function lorenz63_burn_in_steps(spec::Lorenz63Spec)
    return Int(round(spec.burn_in_time / spec.dt))
end

function build_lorenz63_raw_trajectory(
    spec::Lorenz63Spec,
    trajectory_index::Integer,
    sampled_x0::AbstractVector{<:Real},
)
    burn_in_state = advance_lorenz63_state(spec, sampled_x0, lorenz63_burn_in_steps(spec))
    times, X = generate_lorenz63_trajectory(spec, burn_in_state)
...
function validate_lorenz63_raw_trajectory_dimensions(spec::Lorenz63Spec, traj::RawTrajectory)
...
function generate_lorenz63_raw_trajectories(system_config::AbstractDict)
    spec = lorenz63_spec_from_config(system_config)
    validate_lorenz63_spec(spec)
    x0_values = lorenz63_manual_initial_conditions(system_config)

    raw_trajectories = RawTrajectory[]
    for (q, x0) in enumerate(x0_values)
        raw = build_lorenz63_raw_trajectory(spec, q, x0)
        validate_lorenz63_raw_trajectory_dimensions(spec, raw)
...
function generate_lorenz63_observed_trajectories(
...
function build_lorenz63_split(raw_trajectories::AbstractVector{RawTrajectory}, split_config::AbstractDict)
...
function build_lorenz63_window_summary(
    split::AbstractDict,
    ...
    for horizon in Int.(window_config["rollout"]["horizons"])
        rollout_windows = build_rollout_windows(
            split,
            trajectory_length;
            window_id = string("lorenz63_rollout_h", horizon),
            horizon = horizon,
        )
...
function lorenz63_matrix_tensor(matrices::AbstractVector{<:AbstractMatrix})
...
function save_lorenz63_raw(path::AbstractString, raw_trajectories::AbstractVector{RawTrajectory})
    ...
    JLD2.jldsave(
        ...
        state_tensor = lorenz63_matrix_tensor([traj.state_matrix for traj in raw_trajectories]),
        array_layout = "state_dim_by_time_by_trajectory",
    )
    return path
end

function save_lorenz63_observed(
    path::AbstractString,
    ...
    JLD2.jldsave(
        ...
        state_tensor = lorenz63_matrix_tensor([traj.state_matrix for traj in observed_trajectories]),
        observation_tensor = lorenz63_matrix_tensor([traj.observation_matrix for traj in observed_trajectories]),
        array_layout = "state_dim_by_time_by_trajectory",
    )
    return path
end

function lorenz63_csv_value(value)
...
function write_lorenz63_single_row_csv(path::AbstractString, columns::AbstractVector, values::AbstractVector)
    ensure_parent_dir(path)
    open(path, "w") do io
        println(io, join(columns, ","))
        println(io, join(lorenz63_csv_value.(values), ","))
    end
    return path
end

function write_lorenz63_state_ranges_csv(path::AbstractString, diagnostics::AbstractDict)
    ensure_parent_dir(path)
    open(path, "w") do io
        println(io, "coordinate,min,max")
        println(io, "x,", diagnostics["x_min"], ",", diagnostics["x_max"])
        println(io, "y,", diagnostics["y_min"], ",", diagnostics["y_max"])
        println(io, "z,", diagnostics["z_min"], ",", diagnostics["z_max"])
    end
    return path
end

function write_lorenz63_statistics_csv(path::AbstractString, diagnostics::AbstractDict)
    ensure_parent_dir(path)
...
function write_lorenz63_split_window_counts_csv(
    path::AbstractString,
    split::AbstractDict,
...
function maybe_save_lorenz63_plots(
    raw_trajectories::AbstractVector{RawTrajectory},
    ...
    if !isdefined(Main, :PLOTS_AVAILABLE) || !Main.PLOTS_AVAILABLE
        if isdefined(Main, :PLOTS_LOAD_ERROR)
            @warn "Skipping Lorenz63 plots because Plots.jl could not be loaded" exception = Main.PLOTS_LOAD_ERROR[]
        else
            @warn "Skipping Lorenz63 plots because Plots.jl was not imported"
        end
        return String[]
    end
    ...
    try
        ...
        p_phase3d = Main.Plots.plot(;
            ...
            ylabel = "y",
            zlabel = "z",
            title = string("Lorenz63 ", run_label, " phase space"),
            legend = false,
            camera = (45, 25),
        )
        ...
        phase3d_path = joinpath(plot_dir, "lorenz63_phase3d.png")
        ...
        for (name, i, j, xlabel, ylabel) in projections
            p = Main.Plots.plot(;
                xlabel = xlabel,
                ylabel = ylabel,
                title = string("Lorenz63 ", run_label, " ", name, " projection"),
                legend = false,
            )
            for traj in selected
                Main.Plots.plot!(p, traj.state_matrix[i, :], traj.state_matrix[j, :])
            end
            path = joinpath(plot_dir, string("lorenz63_projection_", name, ".png"))
        ...
        p_time = Main.Plots.plot(
            ...
            ylabel = "state",
            label = ["x(t)" "y(t)" "z(t)"],
            title = string("Lorenz63 ", run_label, " time series"),
        )
        time_path = joinpath(plot_dir, "lorenz63_timeseries_xyz.png")
        Main.Plots.savefig(p_time, time_path)
        push!(plot_files, time_path)

        return plot_files
    catch err
        @warn "Skipping Lorenz63 plots because plot generation failed" exception = err
        return String[]
    end
end

function make_lorenz63_manifest(;
    ...
    return Dict(
        ...
        "sigma_default" => spec.sigma,
        "rho_default" => spec.rho,
        "beta_default" => spec.beta,
        "parameter_domain" => system_config["parameter_domain"],
        "dt" => spec.dt,
        "burn_in_time" => spec.burn_in_time,
        "burn_in_steps" => lorenz63_burn_in_steps(spec),
        ...
        "array_layout" => "state_dim_by_time_by_trajectory",
        "system_metadata" => lorenz63_metadata(spec),
...
function save_lorenz63_outputs(;
    ...
    raw_path = lorenz63_project_path(project_root, output_policy["raw_path"])
    processed_path = lorenz63_project_path(project_root, output_policy["processed_path"])
    split_path = lorenz63_project_path(project_root, output_policy["split_path"])
    windows_summary_path = lorenz63_project_path(project_root, output_policy["windows_summary_path"])
    ...
    plot_files = maybe_save_lorenz63_plots(raw_trajectories, plot_dir, run_label)
    ...
    manifest = make_lorenz63_manifest(
        configs = configs,
        spec = spec,
    ...
    release_index = Dict(
        "release_id" => string("lorenz63_fullobs_v1_", run_label),
    ...
    columns, values = lorenz63_diagnostics_csv_row(spec, observation_spec.observation_id, diagnostics)
    write_lorenz63_single_row_csv(table_path, columns, values)
    write_lorenz63_state_ranges_csv(state_ranges_path, diagnostics)
    write_lorenz63_statistics_csv(statistics_path, diagnostics)
    write_lorenz63_split_window_counts_csv(split_window_counts_path, split, window_summary)
    ...
    open(log_path, "w") do io
        ...
        println(io, "state_tensor size: ", size(lorenz63_matrix_tensor([traj.state_matrix for traj in raw_trajectories])))
        ...
        println(io, "divergence: ", diagnostics["divergence"])
        println(io, "wing switch counts: ", diagnostics["wing_switch_counts"])
