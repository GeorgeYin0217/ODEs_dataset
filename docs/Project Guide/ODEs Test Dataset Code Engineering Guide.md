...
此时 $\mathbf{z}=\mathbf{x}$。fileciteturn15file0
...
fileciteturn15file0turn15file1
...
该层只保存声明式配置，不保存生成后的数据。

- `systems/`：系统参数配置；
- `observations/`：观测链配置；
- `splits/`：切分协议配置；
- `windows/`：窗口协议配置；
- `tasks/`：benchmark 任务配置；
- `benchmarks/`：某次实验所需的数据与任务集合；
- `releases/`：正式发布清单。
...
负责把 `SystemSpec + ObservationSpec + Seed` 转换为原始轨线与处理轨线。

### 4.5 `src/datasets/`

负责把轨线组织成统一数据对象，而不是直接交给算法。

### 4.6 `src/splits/`

负责生成 train / val / test 的索引集合。
...
负责统一计算 benchmark 指标，包括误差、统计差异、稳定性与运行代价等。
...
我可以继续把这份文档整理成更接近你 Obsidian 风格的版本，改成 `>[!NOTE]` 分节样式。